To compile the source code on Windows using Visual Studio 2015 (or newer), Go to "Windows Version" sub-folder and load the solution to VS then build the solution. A pre-compiled opsm.exe is also provided.
...
threshold_probabilty: is a parameter for PF and PFA based methods only, please refer to the paper for detailed meaning, generally you can set any value from [0,1], the bigger the value the less amount of OPSMs will be returned.

The main() function is in ydTest.cpp.
The main1() function in ydTest.cpp is the most directly way of calling our methods provided for users' reference. 